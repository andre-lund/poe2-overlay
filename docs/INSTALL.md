...
(required for the layer-shell surface on KDE Wayland) and `APPIMAGE_EXTRACT_AND_RUN=1`
(so the AppImage bundles even without FUSE — e.g. in a container/CI; a normal desktop
with FUSE doesn't need it).
...
2. Hover an item, press **Ctrl+Alt+D** — the overlay shows the cheapest listings.
3. In the card: pick a **league**, toggle **stat / base filters** and edit min/max, then
   **Requery**; **✕** or **Ctrl+Alt+X** dismisses it.
...
rm -f ~/.local/bin/poe2-overlay \
      ~/.local/share/applications/poe2-overlay-{pricecheck,hide}.desktop \
